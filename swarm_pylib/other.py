...
def swarm_planning(
        ...
        filter_waypoints: bool = False) -> tuple:
    ...
    for uav in uav_state:
        initial_position_list.append(uav_state[uav]['initial_position'])

    last_position_list = []
    for uav in uav_state:
        last_position_list.append(uav_state[uav]['last_position'])

    # Create a vector of weights for each UAV.
    # It's value is 1 divided by the number of UAVs
    if weight_list is None:
        n_uavs = len(uav_state.keys()) if len(uav_state.keys()) > 0 else 1
        weight_list = [1/len(uav_state.keys())] * n_uavs

    waypoints, wpt_grid = compute_area(
        np.array(initial_position_list),
        np.array(last_position_list),
...
def filter_waypoints(waypoints: list) -> list:
    """ Filter waypoints. Extract only vortex waypoints
    Args:
